...
class BTree {
    ...
    private void remover(Node no, int chave) {
        ...
        if (index != -1) {
            if (no.folha) {
                ...
                Node antecessor = no.filhos.get(index);
                Node sucessor = no.filhos.get(index + 1);

                if (antecessor.n >= grauMinimo) {
                    int antecessorChave = obterAntecessor(antecessor);
                    no.chaves.set(index, antecessorChave);
                    remover(antecessor, antecessorChave);
                } else if (sucessor.n >= grauMinimo) {
            ...
            if (no.filhos.get(i).n < grauMinimo) {
                preencher(no, i);
            }

            if (ultimoFilho && i > no.n) {
                remover(no.filhos.get(i - 1), chave);
            } else {
    ...
    private void imprimirArvorePorPagina(Node no, int nivel, int pagina) {
        System.out.println("Página " + pagina + ", Nível " + nivel + ": " + no.chaves);
        if (!no.folha) {
